...
def _detect_scenes_python_api(
    ...
    """
    Perform scene detection using the PySceneDetect Python API.

    This method uses a combination of ContentDetector and AdaptiveDetector
    to robustly identify scene changes. It also includes special handling
    for short videos to ensure they are treated as a single scene.

    Args:
        video_path: Path to the input video file.
        output_dir: Directory to save scene-related output files.
        threshold: Detection threshold for the ContentDetector.
        min_scene_len_sec: Minimum duration for a valid scene in seconds.
        save_csv: Whether to save a detailed CSV file of the scenes.
        short_video_threshold_sec: The duration at or below which a video is
            considered "short" and treated as a single scene.

    Returns:
        A list of tuples, where each tuple contains the start and end time
        of a detected scene in seconds.
    """
    from scenedetect import SceneManager, open_video
    from scenedetect.detectors import ContentDetector, AdaptiveDetector
    from scenedetect.stats_manager import StatsManager
...
def _detect_scenes_cli_fallback(video_path: str, output_dir: str) -> List[Tuple[float, float]]:
    """
    Perform scene detection using the `scenedetect` command-line interface.

    This method serves as a fallback for when the PySceneDetect Python API is
    not available. It runs the `scenedetect` command as a subprocess and
    parses the resulting CSV file.

    Args:
        video_path: Path to the input video file.
        output_dir: Directory to save the output from the CLI tool.
...
def _save_scenes_csv(output_dir: str, scene_list_timecodes: List, scene_boundaries_sec: List[Tuple[float, float]]):
    """
    Save the detected scene information to a CSV file.

    The CSV file includes details for each scene, such as start and end
    frames, start and end times in seconds, and the duration.

    Args:
        output_dir: The directory where the CSV file will be saved.
        scene_list_timecodes: The list of scene boundaries as Timecode objects.
        scene_boundaries_sec: The list of scene boundaries in seconds.
    """
    import csv
...
def _parse_scenes_csv(csv_path: str) -> List[Tuple[float, float]]:
    """
    Parse scene boundaries from a CSV file generated by `scenedetect`.
