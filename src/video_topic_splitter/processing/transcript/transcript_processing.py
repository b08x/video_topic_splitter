...
def load_transcript_sentences(path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not isinstance(transcript_data, list):
            logger.warning(f"Expected list of sentences, got {type(transcript_data).__name__}. Attempting to extract sentences.")
...
def merge_sentences_into_paragraphs(sentences: List[Dict[str, Any]], 
                                   max_gap: float = 1.0,
                                   max_paragraph_duration: float = 30.0) -> List[Dict[str, Any]]:
    ...
    for sentence in sentences[1:]:
        ...
        paragraph_duration = current_paragraph["end_time"] - current_paragraph["start_time"]
        
        if (time_gap > max_gap or 
            paragraph_duration > max_paragraph_duration or
            (sentence.get("speaker") is not None and 
             current_paragraph["sentences"][-1].get("speaker") is not None and
             sentence["speaker"] != current_paragraph["sentences"][-1]["speaker"])):
...
def filter_transcript_by_time_range(transcript: List[Dict[str, Any]], 
                                   start_time: float, 
